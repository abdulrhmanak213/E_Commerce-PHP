...
```php
jwtToken::builder(); // Returns an instance of ReallySimpleJWT\Build

jwtToken::parser($token); // Returns an instance of ReallySimpleJWT\Parse

jwtToken::validator($token, $secret); // Returns an instance of ReallySimpleJWT\Validate
```
...
$app->get('/jwt', function (Request $request, Response $response) {
    $response->getBody()->write("JSON Web jwtToken is Valid!");
